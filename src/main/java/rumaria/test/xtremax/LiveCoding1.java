...
public class LiveCoding1 {
  ...
  /**
   * Special Number Special Number is list of non-negative integer that is constructed from
   * non-negative integer Special Number has two operations, Special Summation and Special
   * Multiplication
   * <p>
   * You must implement specialSummation and Special Multiplication Method See explanation for each
   * method below
   * <p>
  ...
  private static void testResult(int expected, int actual) {
    if (expected == actual) {
      ...
      System.out.println(
          String.format("Answer is wrong. Expected level is %d but your result is %d", expected,
              actual)
      );
    }
  }
  ...
  /**
   * specialSummation is sums of all digit in Special Number Example: >>> specialSummation(123) >>>
   * 6   (from 1+2+3)
   * <p>
   * >>> specialSummation(567) >>> 9   (from 5+6+7 = 18 then 1+8 = 9)
   *
  ...
  /**
   * specialMultiplication is multiplication of all digit in Special Number Example: >>>
   * specialMultiplication(123) >>> 6   (from 1*2*3)
   * <p>
   * >>> specialSummation(567) >>> 0   (from 5*6*7 = 210 then 2*1*0 = 0)
   *
